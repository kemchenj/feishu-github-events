...
test("renders default template for every supported action event", () => {
  for (const eventName of ACTION_EVENTS) {
    const event = normalizeGitHubEvent(eventName, createMockPayload(eventName));
...
test("renders summary without default context fields", () => {
  const event = normalizeGitHubEvent("push", createMockPayload("push"));
  const message = renderTemplate("default", event);
  const [summary] = message.card.elements;
  const body = JSON.stringify(message.card.elements);
...
test("renders single comment summaries without list markers", () => {
  ...
  const message = renderTemplate("default", event);
  const summary = message.card.elements.find((element) => element.text?.content);
...
test("preserves markdown in comment summaries without links", () => {
  ...
  const event = normalizeGitHubEvent("pull_request_review_comment", payload);
  const message = renderTemplate("default", event);
  const summary = message.card.elements.find((element) => element.text?.content);
...
test("renders single key-value summaries without list markers", () => {
  const event = normalizeGitHubEvent("issues", createMockPayload("issues"));
  const message = renderTemplate("default", event);
  const summary = message.card.elements.find((element) => element.text?.content);
...
test("renders workflow call inputs as a yaml-like summary block", () => {
  const event = normalizeGitHubEvent("workflow_call", createMockPayload("workflow_call"));
  const message = renderTemplate("default", event);
...
test("omits manual workflow inputs by default", () => {
  const event = normalizeGitHubEvent("workflow_dispatch", createMockPayload("workflow_dispatch"));
  const message = renderTemplate("default", event);
  const body = JSON.stringify(message.card.elements);
...
test("omits repository dispatch payload by default", () => {
  const event = normalizeGitHubEvent("repository_dispatch", createMockPayload("repository_dispatch"));
  const message = renderTemplate("default", event);
  const body = JSON.stringify(message.card.elements);
...
test("renders branch deletion with a red header", () => {
  const event = normalizeGitHubEvent("delete", createMockPayload("delete"));
  const message = renderTemplate("default", event);
...
test("uses one consistent button label by default", () => {
  for (const eventName of ACTION_EVENTS) {
    const event = normalizeGitHubEvent(eventName, createMockPayload(eventName));
    const message = renderTemplate("default", event);
    const action = message.card.elements.find((element) => element.tag === "action");
