...
test("builds push payload message", () => {
...
test("normalizes pull request payload", () => {
  const event = buildGitHubEventMessage("pull_request", createMockPayload("pull_request"));
...
test("falls back to the workflow run URL when an event has no detail URL", () => {
  const event = buildGitHubEventMessage("public", createMockPayload("public"), {
...
test("distinguishes commit status payloads by context and state", () => {
  const event = buildGitHubEventMessage("status", createMockPayload("status"));
  const failure = buildGitHubEventMessage("status", createMockPayload("status", "failure"));
  ...
  assert.equal(failure.headerTemplate, "red");
});

test("writes specific pull request review titles", () => {
  const review = buildGitHubEventMessage(
    "pull_request_review",
    createMockPayload("pull_request_review")
  );
  const comment = buildGitHubEventMessage(
...
test("does not repeat wiki page names in the body", () => {
  const event = buildGitHubEventMessage("gollum", createMockPayload("gollum"));
...
test("keeps comment titles short and truncates comment bodies", () => {
  ...
  const event = buildGitHubEventMessage("issue_comment", payload);
...
test("shows workflow conclusions in the title", () => {
  const success = buildGitHubEventMessage("workflow_run", createMockPayload("workflow_run"));
  const failure = buildGitHubEventMessage("workflow_run", createMockPayload("workflow_run", "failure"));
...
test("does not repeat package type when package title is already specific", () => {
  const event = buildGitHubEventMessage("registry_package", createMockPayload("registry_package"));
...
test("distinguishes deployment success and failure", () => {
  const success = buildGitHubEventMessage("deployment_status", createMockPayload("deployment_status"));
  const failure = buildGitHubEventMessage("deployment_status", createMockPayload("deployment_status", "failure"));

  assert.equal(success.title, "Deployment succeeded from main: production");
  assert.equal(success.headerTemplate, "green");
  assert.equal(failure.title, "Deployment failed from main: production");
  assert.equal(failure.headerTemplate, "red");
  assert.match(failure.summary[0].text, /Health checks failed/);
});
