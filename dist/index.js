...
function buildWorkflowRunEvent(payload) {
    ...
    return {
        title: titleWithDetails(titleWithRef(`Workflow ${workflowRunState(payload)}`, payload.workflow_run?.head_branch), payload.workflow_run?.name)
    };
}
...
function buildDeploymentEvent(payload) {
    return {
        title: titleWithDetails(titleFromRef("Deployment requested", payload.deployment?.ref), payload.deployment?.environment)
    };
}
function buildDeploymentStatusEvent(payload) {
    ...
    return {
        title: titleWithDetails(titleFromRef(`Deployment ${deploymentState(state)}`, payload.deployment?.ref), payload.deployment_status?.environment || payload.deployment?.environment)
    };
}
...
function buildCheckRunEvent(payload) {
    ...
    return {
        title: titleWithDetails(titleWithRef(`Check run ${state}`, payload.check_run?.check_suite?.head_branch), payload.check_run?.name),
...
function buildGollumEvent(payload) {
    return {
        title: `Wiki updated: ${(payload.pages || []).map((page) => page.page_name).filter(Boolean).join(", ")}`
    };
}
function buildImageVersionEvent(payload) {
    return {
        title: `Image version ${payload.action || ""}: ${compact$1([payload.image_version?.name, payload.image_version?.version]).join(" ")}`.trim()
    };
}
...
function buildMergeGroupEvent(payload) {
    return {
        title: titleWithDetails(`Merge group ${actionLabel(payload.action, { checks_requested: "checks requested" })}`, payload.merge_group?.base_ref || payload.merge_group?.head_ref)
    };
}
...
function checkState(check, fallbackAction) {
    return (resultState(check?.conclusion) ||
...
function issueCommentTitle(payload) {
    ...
    return titleWithDetails(`${kind} #${issue.number || ""} comment${commentTitleSuffix(payload.action)}`, truncate(issue.title || "", 60));
}
...
function pullRequestReviewCommentTitle(payload) {
    const pullRequest = payload.pull_request || {};
    return titleWithDetails(`PR #${pullRequest.number || ""} review comment${commentTitleSuffix(payload.action)}`, truncate(payload.comment?.path || "", 60));
}
